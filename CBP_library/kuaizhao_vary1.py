...
def calculate_metrics(points):
    ...
    x = points[::2]
    y = points[1::2]
    return (np.mean(x), np.mean(y)), np.ptp(x), np.ptp(y)
...
def transform_leaf(args):
    ...
    if enable_scale:
        ...
        y_scale = x_scale#np.random.uniform(0.97, 1.03)
...
def batch_transform(data, cx, cy, x_span, y_span, enable_scale=False):
    """多进程批量处理（新增enable_scale参数）"""
    with Pool(processes=cpu_count()) as pool:
        args = [(i, leaf, cx, cy, x_span, y_span, enable_scale)
...
if __name__ == "__main__":
    ...
    # 计算基准参数
    (cx, cy), x_span, y_span = calculate_metrics(all_leaves[0])

    # 执行变换（启用缩放功能）
    transformed = batch_transform(all_leaves, cx, cy, x_span, y_span, enable_scale=True)

    # 保存结果
    save_data(transformed, "kuaizhao_vary1.txt")
    print("处理完成！结果已保存至 kuaizhao_vary1.txt")